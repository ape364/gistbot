...
class ShelveHelper:
    ...
    def get_last_message_ts(self, user_id):
        return int(self.__get_value(user_id).get('date'))

    def get_last_text_message_hash(self, user_id):
        return self.__get_value(user_id).get('txt_msg_hash')

    def get_last_file_contents_hash(self, user_id):
        return self.__get_value(user_id).get('file_contents_hash')

    def get_last_file_id(self, user_id):
        return self.__get_value(user_id).get('file_id')

    def get_last_gist_from_text_url(self, user_id):
        return self.__get_value(user_id).get('text_url')

    def get_last_gist_from_file_url(self, user_id):
        return self.__get_value(user_id).get('file_url')

    def update_user_history(self, user_id, **kwargs):
        d = dict.fromkeys(self.__get_keys())
        for k in self.__get_keys():
            if k in kwargs:
                d[k] = kwargs[k]
            else:
                d[k] = self.__get_value(user_id).get(str(k))
        self.__set_value(user_id, d)
